...
function renderBlocks() {
  const editor = document.getElementById('scriptPage').parentElement;
...
function createPage() {
  ...
  page.className = 'script-page generated-page';
  const body = document.createElement('div');
  body.className = 'script-page-body';
...
function renderStats() {
  ...
  const pages = Math.max(1, document.querySelectorAll('.editor-area .script-page').length);
  document.getElementById('statScenes').textContent = `${scenes} scenes`;
  document.getElementById('statWords').textContent = `${words} words`;
  document.getElementById('statPages').textContent = `${pages} ${pages === 1 ? 'page' : 'pages'}`;
}
